<!-- API-MODE ENGINE CORE. Derived from SKILL.md, which remains the installable Claude-skill
distribution (kept byte-identical as a fallback / high-context ad-hoc path). Only Step 1 (ingest)
and Step 7 (emit) differ here — reframed for a single API response where ingest already ran in code
and there are no file-writing tools. Steps 2–6, the nine-category taxonomy, pattern rules, run
scope, and guardrails are carried over from SKILL.md VERBATIM; keep them diff-identical when either
file changes. This file is what config.load_engine_core() injects into the prompt. -->

# Sentiment Engine — audience comments → creator insight (purpose- + client-configurable)

This engine core turns a set of audience comments (optionally plus a video transcript) into a structured
...
The only deterministic seam is **ingest + provenance** — run in code before this prompt, so the
model never hand-parses a CSV or invents an ID. Everything between ingest and emit is your
...
### Step 1 — Ingest (already done, deterministically, before this prompt)

Ingest has already run **deterministically, in code, outside this prompt** — you are not being asked
to run it. The comments provided under the run inputs are already normalised to the canonical row
shape (`SYSTEM_INTEGRATION_CONTRACT.md` §3.1), their `yt:` `source_id`s are already derived, and the
provenance block is already assembled. **Use the provided normalised rows and provenance verbatim** —
the normalised comments are the set you analyse; the provenance block is what you carry into
`summary.json` at Step 7.

Do not re-derive, re-parse, or re-normalise the comments to route around a perceived problem — that
consistency is exactly what the deterministic seam exists to protect. If the provided rows look wrong
against contract §3.1 (a missing `source_id`/`comment_id`, or a comment whose comma/quote/newline
clearly didn't round-trip), **say so in the report** rather than silently reshaping the data — a quiet
mismatch here is the one thing that bites the handoff.
...
### Step 7 — Emit both outputs (one API response)

This is a **single API response**, not a Claude Code session with file-writing tools. Emit **both**
outputs in this one message, in this order — nothing before the report, nothing after the closing fence:
...
1. **The human Markdown report** — structured exactly per the active purpose's `output_structure`.
   ...
   **verbatim** audience quotes into the report as evidence (and into the summary where used).
   **British English throughout.**
2. A line containing only `===SUMMARY_JSON===`.
3. **The machine `summary.json`** — a single fenced ```json code block, per
   `SYSTEM_INTEGRATION_CONTRACT.md` §4. It carries the **provenance block** (from Step 1, copied
   **verbatim** as a nested object — never flattened into top-level keys), the `purpose` field, and
   the findings made queryable: `themes`, `question_clusters`, `pain_points`, `gaps`, plus any
   `summary_json_extensions` the purpose adds. Every array item's identifying field is named `name`
   (not `theme`/`gap`/`pain_point`/etc). The exact schema is given with the run inputs below.
...
---
*Engine core v1.0 — conforms to SYSTEM_INTEGRATION_CONTRACT (shared spine §1, comments CSV §3.1, summary.json §4) and the sentiment purpose+client design spec. Agnostic core; strategy lives in the configs.*
